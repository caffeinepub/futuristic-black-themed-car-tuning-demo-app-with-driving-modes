...
/**
 * Gets the currently selected section from sessionStorage
 * Defaults to 'tuning' if no section is stored
 *
 * @returns The selected section ID
 */
export function getSelectedSection(): string {
    return getSessionParameter('selectedSection') || 'tuning';
}

/**
 * Stores the selected section in sessionStorage for persistence
 *
 * @param sectionId - The section ID to store
 */
export function setSelectedSection(sectionId: string): void {
    storeSessionParameter('selectedSection', sectionId);
}
